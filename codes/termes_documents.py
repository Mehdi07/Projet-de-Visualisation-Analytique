...
def main(graph): 
  ...
  for i in range(0,len(R)):
    ...
    R = R.rename(index={i : acronyme[i] })
  
  
  for j in range(0,np.shape(R)[1]):
     R = R.rename(columns={j : vectorizer.get_feature_names()[j]})
  ...
  for e in range(0,np.shape(R)[1]):
    e = G.addNode()
    viewLabel[e] = R.columns[b]
  ...
  for c in range(0,np.shape(R)[1]):
      for d in range(0,len(R)):
        if R[R.columns[c]][acronyme[d]] == 1:
          o = o +1
          G.addEdge(dic[R.columns[c]],dic[acronyme[d]])
