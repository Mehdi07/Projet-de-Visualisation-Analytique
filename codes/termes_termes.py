...
def main(graph): 
  ...
  df = pd.read_csv('C:/Users/Mehdi/Desktop/M1/Visualisation/data/city_df.txt',sep ='\t' , encoding='utf-8')
  ...
  vectorizer = CountVectorizer(stop_words=stop_words, min_df=0.02, max_df=0.03, ngram_range=(1,1))
  ...
  for i in range(0,len(R)):
    R = R.rename(index=int,columns={i : vectorizer.get_feature_names()[i]})
  
  b = 0;
  
  for a in range(0,len(tt)):
    a = G.addNode()
  ...
  for c in range(0,len(R)):
      for d in range(c+1,len(R)):
        if R[vectorizer.get_feature_names()[c]][d] == 1:
          o = o +1
